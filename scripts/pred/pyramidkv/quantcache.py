...
def fp8_quantize(tensor, axis, group_size):
    """
    Per-group FP8-style quantization using linear scaling.
    """
    # input shape [1, 8,4k,128]
    tar_dtype = torch.float8_e4m3fn
    org_dtype = tensor.dtype
    print(org_dtype)
    qmin, qmax = torch.finfo(tar_dtype).min, torch.finfo(tar_dtype).max
    
    if axis == 1:
        tensor_reshaped = tensor.reshape([-1, group_size]) # head_dim
        absmax = tensor.abs().amax(dim=-1, keepdim=True)  # shape [num_groups, 1]
    else:
        tensor_reshaped = tensor.reshape([group_size, -1]) # token_dim
        absmax = tensor.abs().amax(dim=0, keepdim=True)  # shape [1，num_groups]
    
    # per-tensor quantization
    # absmax = tensor.max()
    
    scale = (absmax / qmax).clamp(min=1e-5)

    qtensor = (tensor_reshaped / scale).clamp(qmin, qmax).to(tar_dtype).to(org_dtype)
    # cal the difference between qtensor and tensor dtype firstly
    meta = {
        "scale": scale,
        "axis": axis,
...
def fp8_dequantize(qtensor, meta):
    """
    Dequantize from FP8 using stored scales.
    """
    ...
    dqtensor = qtensor * scales
    return dqtensor.reshape(orig_shape)


class FP8QuantizedCache(QuantizedCache):
    """
    FP8 Quantized Cache:
    - Uses 8-bit float8_e4m3fn quantization with per-group scaling
    - Follows exact config pattern of HQQQuantizedCache
    """
    ...
    def __init__(self, cache_config: CacheConfig) -> None:
        ...
        if self.nbits != 8:
            raise ValueError(f"`nbits` for `SimulatedFP8` must be `8`, but got {self.nbits}")

        if self.axis_key not in [0, 1]:
            raise ValueError(f"`axis_key` must be 0 or 1, got {self.axis_key}")

        if self.axis_value not in [0, 1]:
            raise ValueError(f"`axis_value` must be 0 or 1, got {self.axis_value}")

    def _quantize(self, tensor, axis):
        print(tensor.shape)
        qtensor, meta = fp8_quantize(
            tensor,
            axis=axis,
            group_size=self.q_group_size,
        )
    ...
    def _dequantize(self, qtensor):
        ...
        return fp8_dequantize(quant_tensor, meta)
